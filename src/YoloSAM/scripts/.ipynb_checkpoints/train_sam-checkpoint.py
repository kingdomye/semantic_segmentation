    ...
    def train_epoch(self) -> float:
        ...
        for batch_idx, batch in enumerate(progress_bar):
            ...
            if batch_idx == 0 and self.current_epoch == 0:
                ...
                print(f"输入mask求和: {masks.sum().item()}")
                # 修复：从batch中读取boxes
                boxes = batch['boxes'].to(self.device)
                print(f"输入boxes形状: {boxes.shape}, 值: {boxes}")
            ...
            for i in range(len(images)):                
                ...
                if boxes is not None and len(boxes) > i:
                    prompt_data['boxes'] = boxes[i:i+1]  # 保持[1,4]维度
            
                # 核心：移出缩进，强制每个样本执行前向传播
                pred_mask, _ = self.model.forward_one_image(
                    image=images[i:i+1],
                    points=prompt_data.get('points'),
                    bounding_box=prompt_data.get('boxes'),
            ...
            # if batch_idx == 0 and self.current_epoch == 0:
            #     print(f"预测掩码求和: {pred_masks.sum().item()}, 二值化后求和: {pred_masks_binary.sum().item()}")
            ...
            pred_masks_logits = torch.cat(batch_pred_masks, dim=0) # <-- 重命名为 logits

            # 🔥 关键修改：先用 logits 计算 Loss
            loss = self.criterion(pred=pred_masks_logits, target=masks)
            batch_loss = loss
            
            # 🔥 关键修改：只在计算 Dice 时才用 Sigmoid
            pred_masks_prob = torch.sigmoid(pred_masks_logits)
            pred_masks_binary = (pred_masks_prob > 0.5).float()
    
            if batch_idx == 0 and self.current_epoch == 0:
                # 日志现在打印 prob 的求和
                print(f"预测掩码(概率)求和: {pred_masks_prob.sum().item()}, 二值化后求和: {pred_masks_binary.sum().item()}")
    ...
    def validate(self) -> float:
        ...
        with torch.no_grad():
            for batch in tqdm(self.val_loader, desc='Validation'):
                ...
                dice = self.dice_metric(pred_masks_binary, masks)
                # dice_scores.append(dice.mean().item()) # 你的 dice.mean().item() 可能会报错，因为 calculate_dice 已经返回了 item()
                dice_scores.append(dice.mean().item())
